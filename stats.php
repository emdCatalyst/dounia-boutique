...
$stmt = $pdo->prepare("SELECT SUM(total_amount - delivery_price) as total, 
                               SUM(total_amount - delivery_price - (COALESCE(o.product_cost, CAST(p.description AS DECIMAL(10,2))) * o.quantity)) as profit
...
$stmt = $pdo->prepare("SELECT SUM(total_amount) as total, 
                              SUM(total_amount - (COALESCE(s.product_cost, CAST(p.description AS DECIMAL(10,2))) * s.quantity)) as profit 
